    ...
    exports.load = function (contents) {
      ...
      return function tokenize(text2) {
        ...
        let pushToken = (word) => {
          ...
          column =
            lastLineBreakIndex >= 0 ? word.length - lastLineBreakIndex : column + word.length;
        };
          ...
          if (i !== text2.length - 1) {
            ...
            for (let entries of [traditionalEntries, simplifiedEntries]) {
              for (let entry of entries) {
                let matchText =
                  entries === traditionalEntries ? entry.traditional : entry.simplified;
      ...
      return objects.reduce(function (result, current) {
        ...
        Object.keys(current).forEach(function (key) {
          ...
          if (Array.isArray(result[key]) && Array.isArray(current[key])) {
            result[key] = merge2.options.mergeArrays
...
var EnglishOnlyTokenizer = class extends DefaultTokenizer {
  tokenize(content, raw) {
    ...
    return raw
      ...
      : tokenized.map((x) => x.word).filter((x) => !x.match(this.getTrimPattern()));
  }
  recursiveTokenize(content) {
...
var AppHelper = class {
  ...
  getFrontMatter(file) {
    ...
    const tags =
      (_c =
        ...
        : [];
    const aliases =
      (_d = (0, import_obsidian.parseFrontMatterAliases)(frontMatter)) != null ? _d : [];
  ...
  getCurrentEditor() {
    var _a, _b;
    return (_b = (_a = this.getMarkdownViewInActiveLeaf()) == null ? void 0 : _a.editor) != null
      ? _b
      : null;
  }
  getSelection() {
  ...
  optimizeMarkdownLinkText(linkText) {
    ...
    if (markdownLink.startsWith('[[')) {
      const text2 =
        (_a = markdownLink.matchAll(/^\[\[(?<text>.+)]]$/g).next().value.groups) == null
          ? void 0
          : _a.text;
  ...
  searchPhantomLinks() {
    return Object.entries(this.unsafeApp.metadataCache.unresolvedLinks).flatMap(([path, obj]) =>
      Object.keys(obj).map((link) => ({ path, link }))
    );
  }
  getUnresolvedLinks(file) {
    var _a;
    const countsByLink =
      (_a = this.unsafeApp.metadataCache.unresolvedLinks[file.path]) != null ? _a : {};
  ...
  isIMEOn() {
    ...
    return !!((_c = (_b = cm5or6 == null ? void 0 : cm5or6.display) == null ? void 0 : _b.input) ==
    null
      ? void 0
      : _c.composing);
  }
...
WordTypeMeta._dict = {};
WordTypeMeta.FRONT_MATTER = new _WordTypeMeta('frontMatter', 100, 'frontMatter');
WordTypeMeta.INTERNAL_LINK = new _WordTypeMeta('internalLink', 90, 'internalLink');
WordTypeMeta.CUSTOM_DICTIONARY = new _WordTypeMeta('customDictionary', 80, 'suggestion');
WordTypeMeta.CURRENT_FILE = new _WordTypeMeta('currentFile', 70, 'suggestion');
WordTypeMeta.CURRENT_VAULT = new _WordTypeMeta('currentVault', 60, 'suggestion');
...
function judge(word, query, queryStartWithUpper, fuzzy) {
  ...
  const matchedAlias =
    (_a = word.aliases) == null
      ? void 0
      : _a.map((a) => ({ aliases: a, matched: matcher(a, query) })).find((x) => x.matched);
  ...
  const words = queryStartWithUpper
    ? frontMatter
      ...
      : [
          ...
          ...((_g = indexedWords.customDictionary[query.charAt(0).toLowerCase()]) != null
            ? _g
            : []),
...
function judgeByPartialMatch(word, query, queryStartWithUpper, fuzzy) {
  ...
  const matchedAliasIncluded =
    (_b = word.aliases) == null
      ? void 0
      : _b.map((a) => ({ aliases: a, matched: includesMatcher(a, query) })).find((x) => x.matched);
...
function basename(path, ext) {
  var _a, _b;
  const name =
    (_b = (_a = path.match(/.+[\\/]([^\\/]+)[\\/]?$/)) == null ? void 0 : _a[1]) != null
      ? _b
      : path;
  ...
  refreshWords(option) {
    ...
    const resolvedInternalLinkWords = this.app.vault
      ...
      .filter((f) => {
        ...
        return !this.appHelper.getBoolFrontMatter(f, option.frontMatterKeyForExclusion);
      })
      .flatMap((x) => {
...
MatchStrategy.PARTIAL = new _MatchStrategy('partial', suggestWordsByPartialMatch);
  ...
  async refreshWords(option) {
    ...
    for (const path of markdownFilePaths) {
      ...
      const tokens = this.tokenizer
        ...
        .filter(
          (x) =>
            x.length >= option.minNumberOfCharacters && !this.tokenizer.shouldIgnoreOnCurrent(x)
        )
        .map((x) => (startsSmallLetterOnlyFirst(x) ? x.toLowerCase() : x));
      for (const token of tokens) {
        wordByValue[token] = {
...
DescriptionOnSuggestion.FULL = new _DescriptionOnSuggestion('Full', (word) => {
  var _a;
...
function pickWords(file, fm) {
  return Object.entries(fm)
    .filter(
      ([_key, value]) =>
        value != null && (typeof value === 'string' || typeof value[0] === 'string')
    )
...
SpecificMatchStrategy.INHERIT = new _SpecificMatchStrategy('inherit', neverUsedHandler);
SpecificMatchStrategy.PREFIX = new _SpecificMatchStrategy('prefix', suggestWords);
SpecificMatchStrategy.PARTIAL = new _SpecificMatchStrategy('partial', suggestWordsByPartialMatch);
...
var SelectionHistoryStorage = class {
      ...
      for (const value of Object.keys(this.data[hit])) {
        for (const kind of Object.keys(this.data[hit][value])) {
          if (
            ...
            now - this.data[hit][value][kind].lastUpdated > this.maxDaysToKeepHistory * DAY
          ) {
  ...
  getSelectionHistory(word) {
    var _a, _b;
    return (_b = (_a = this.data[word.hit]) == null ? void 0 : _a[word.value]) == null
      ? void 0
      : _b[word.type];
  }
  increment(word) {
...
var AutoCompleteSuggest = class extends import_obsidian3.EditorSuggest {
  ...
  get currentFileMinNumberOfCharacters() {
    return (
      this.settings.currentFileMinNumberOfCharacters || this.tokenizerStrategy.indexingThreshold
    );
  }
  get currentVaultMinNumberOfCharacters() {
    return (
      this.settings.currentVaultMinNumberOfCharacters || this.tokenizerStrategy.indexingThreshold
    );
  }
  ...
  async refreshCustomDictionaryTokens() {
    ...
    await this.customDictionaryWordProvider.refreshCustomWords({
      ...
      delimiterForDisplay:
        this.settings.delimiterToDivideSuggestionsForDisplayFromInsertion || void 0,
    ...
    this.showDebugLog(() =>
      buildLogMessage('Index custom dictionary tokens', performance.now() - start)
    );
  }
  ...
  updateFrontMatterTokenIndex(file) {
    ...
    this.showDebugLog(() =>
      buildLogMessage('Update front matter token index', performance.now() - start)
    );
  }
  ...
  onTrigger(cursor, editor, file) {
    ...
    if (
      currentTokenSeparatedWhiteSpace === this.pastCurrentTokenSeparatedWhiteSpace &&
      !this.runManually
    ) {
    ...
    if (
      currentFrontMatter &&
      ((_c = currentTokens.last()) == null ? void 0 : _c.word.match(/[^ ] $/))
    ) {
    ...
    return {
      start: {
        ch:
          cursor.ch -
          ((_f =
            (_e = (_d = currentTokens.last()) == null ? void 0 : _d.word) == null
              ? void 0
              : _e.length) != null
            ? _f
            : 0),
  ...
  selectSuggestion(word, evt) {
    ...
    if (this.appHelper.equalsAsEditorPosition(this.context.start, this.context.end)) {
      editor.setCursor(
        editor.offsetToPos(editor.posToOffset(editor.getCursor()) + insertedText.length)
      );
    }
...
var DEFAULT_SETTINGS = {
  ...
  customDictionaryPaths: `https://cdn.jsdelivr.net/gh/first20hours/google-10000-english/master/google-10000-english-no-swears.txt`,
  columnDelimiter: 'Tab',
...
var VariousComplementsSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  async addMainSettings(containerEl) {
    ...
    new import_obsidian4.Setting(containerEl).setName('Strategy').addDropdown((tc) =>
      tc
    ...
    new import_obsidian4.Setting(containerEl).setName('Match strategy').addDropdown((tc) =>
      tc
    ...
    new import_obsidian4.Setting(containerEl).setName('Max number of suggestions').addSlider((sc) =>
      sc
        .setLimits(1, 255, 1)
    ...
    new import_obsidian4.Setting(containerEl)
      ...
      .addToggle((tc) => {
        tc.setValue(this.plugin.settings.insertAfterCompletion).onChange(async (value) => {
    ...
    new import_obsidian4.Setting(containerEl)
      ...
      .addText((cb) => {
        cb.setValue(this.plugin.settings.firstCharactersDisableSuggestions).onChange(
          async (value) => {
    ...
    new import_obsidian4.Setting(containerEl)
      ...
      .addToggle((tc) => {
        tc.setValue(this.plugin.settings.useCommonPrefixCompletionOfSuggestion).onChange(
          async (value) => {
  ...
  addKeyCustomizationSettings(containerEl) {
    ...
    new import_obsidian4.Setting(containerEl).setName('Select a suggestion key').addDropdown((tc) =>
      tc
    ...
    new import_obsidian4.Setting(containerEl)
      ...
      .addToggle((tc) => {
        tc.setValue(this.plugin.settings.disableUpDownKeysForCycleThroughSuggestionsKeys).onChange(
          async (value) => {
    ...
    new import_obsidian4.Setting(containerEl).setName('Open source file key').addDropdown((tc) =>
      tc
  ...
  addCustomDictionaryComplementSettings(containerEl) {
    ...
    new import_obsidian4.Setting(containerEl)
      ...
      .addToggle((tc) => {
        tc.setValue(this.plugin.settings.enableCustomDictionaryComplement).onChange(
          async (value) => {
    ...
    if (this.plugin.settings.enableCustomDictionaryComplement) {
      ...
      new import_obsidian4.Setting(containerEl).setName('Column delimiter').addDropdown((tc) =>
        tc
      ...
      new import_obsidian4.Setting(containerEl)
        ...
        .addText((cb) => {
          cb.setValue(this.plugin.settings.customDictionaryWordRegexPattern).onChange(
            async (value) => {
      ...
      new import_obsidian4.Setting(containerEl)
        ...
        .addText((cb) => {
          cb.setValue(this.plugin.settings.caretLocationSymbolAfterComplement).onChange(
            async (value) => {
  ...
  addFrontMatterComplementSettings(containerEl) {
    ...
    if (this.plugin.settings.enableFrontMatterComplement) {
      ...
      new import_obsidian4.Setting(containerEl)
        ...
        .addToggle((tc) => {
          tc.setValue(this.plugin.settings.insertCommaAfterFrontMatterCompletion).onChange(
            async (value) => {
  ...
  addDebugSettings(containerEl) {
    ...
    new import_obsidian4.Setting(containerEl)
      ...
      .addToggle((tc) => {
        tc.setValue(this.plugin.settings.showLogAboutPerformanceInConsole).onChange(
          async (value) => {
      ...
      if (default_slot) {
        if (default_slot.p && (!current || dirty & 4)) {
          update_slot_base(
            ...
            ctx2[2],
            !current
              ? get_all_dirty_from_scope(ctx2[2])
              : get_slot_changes(default_slot_template, ctx2[2], dirty, null),
            null
          );
        }
      }
  ...
  return {
    c() {
      ...
      attr(
        ...
        'class',
        (button_class_value =
          null_to_empty(ctx[1] ? 'button-disabled' : 'button-enabled') + ' svelte-12yh6aw')
      );
    ...
    p(ctx2, [dirty]) {
      ...
      if (
        ...
        (dirty & 2 &&
          button_class_value !==
            (button_class_value =
              null_to_empty(ctx2[1] ? 'button-disabled' : 'button-enabled') + ' svelte-12yh6aw'))
      ) {
...
function create_each_block(ctx) {
  ...
  return {
    ...
    p(ctx2, dirty) {
      if (dirty & 32 && t0_value !== (t0_value = ctx2[26].path + '')) set_data(t0, t0_value);
...
function create_fragment4(ctx) {
  ...
  return {
    c() {
      ...
      if (ctx[2] === void 0) add_render_callback(() => ctx[17].call(select));
    ...
    p(ctx2, [dirty]) {
      ...
      if (!current || dirty & 1024) set_data(t6, ctx2[10]);
      if (dirty & 1) {
      ...
      if (dirty & 4096) obsidianbutton_changes.disabled = !ctx2[12];
      if (dirty & 536870912) {
...
var VariousComponents = class extends import_obsidian6.Plugin {
  ...
  async onload() {
    ...
    const debouncedSaveData = (0, import_obsidian6.debounce)(async () => {
      ...
      await this.appHelper.saveJson(
        ...
        (_b = (_a = this.suggester.selectionHistoryStorage) == null ? void 0 : _a.data) != null
          ? _b
          : {}
      );
    }, 5e3);
  ...
  async loadSettings() {
    ...
    this.settings = (0, import_ts_deepmerge.default)(
      DEFAULT_SETTINGS,
